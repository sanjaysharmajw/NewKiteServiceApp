...
import 'package:nirbhaya_service/Models/video_attend_models.dart';
...
class VideoAttendController extends GetxController{
  final headerController=Get.put(HeaderController());

  Future<VideoAttendModels?> videoAttendApi(VideoAttendRequest request) async {
    try {
      CustomLoader.showLoader("Please wait");
      final response = await http.post(Uri.parse(APIConstant.videoAttend),
      ...
      if (response.statusCode == 200) {
        ...
        return VideoAttendModels.fromJson(responseBody);
      }
    }
